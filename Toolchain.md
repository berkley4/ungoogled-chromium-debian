...
Export your clang bin directory, for example :-

```sh
export LLVM_DIR=/usr/lib/llvm-24/bin
```
...
cmake -B build -G Ninja -S llvm -C clang/cmake/caches/BOLT-PGO.cmake \
...
-DCOMPILER_RT_BUILD_SANITIZERS=ON -DCOMPILER_RT_BUILD_XRAY=OFF -DCOMPILER_RT_SANITIZERS_TO_BUILD="asan;tsan;safestack;cfi" \
-DCOMPILER_RT_USE_LIBCXX=NO -DLLVM_BUILD_LLVM_DYLIB=ON -DLLVM_LINK_LLVM_DYLIB=ON -DBOOTSTRAP_LLVM_ENABLE_LLD=ON \
-DBOOTSTRAP_BOOTSTRAP_LLVM_ENABLE_LLD=ON -DPGO_INSTRUMENT_LTO=Thin -DLLVM_PARALLEL_LINK_JOBS=1 -DLLVM_INCLUDE_TESTS=OFF
```
