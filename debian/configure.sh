...
## vvvvv Holding pen for stable (applies to both unstable & stable) vvvvv
...
sys_patches="libaom-headers openh264"
sys_patches="$(echo $sys_patches | sed "s@\([^ ]*\)@system/unstable/\1@g")"

optional_patches="$optional_patches $sys_patches"
optional_deps="$optional_deps libaom libavif libopenh264 libxslt1"

gn_enable="$gn_enable use_system_libpng"

# SYS_LIBS += libaom libavif libpng libxml libxslt openh264
sys_enable="$sys_enable libaom libavif libpng libxml libxslt openh264"

## ^^^^^ Holding pen for stable (applies to both unstable & stable) ^^^^^
