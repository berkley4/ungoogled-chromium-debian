...
if [ $QT -eq 0 ]; then
  if [ $GTK -eq 0 ]; then
    printf '%s\n' "ERROR: Cannot set QT=0 and GTK=0 (enable at least one)"
    exit 1
  fi
...
if [ $SYS_CLANG -eq 0 ]; then
  ...
  if [ $SYS_CLANG -eq 1 ]; then
    ...
    sed -e "s@=lld@=lld-$LLVM_VER@" \
        -i $DEBIAN/patches/various/fixes/rust-use-lld.patch
  fi
...
if [ $SYS_GN -eq 1 ]; then
  deps_enable="$deps_enable generate-ninja"
...
DSB="$DSB -e \"/^third_party\/libwebp\/BUILD\.gn/d\""
...
## Shell launcher
/bin/sh $M_DIR/update_launcher.sh < $M_DIR/chromium.sh > $M_DIR/chromium
